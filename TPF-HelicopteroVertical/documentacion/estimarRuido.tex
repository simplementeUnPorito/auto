% ============================================================
\section{Apéndice: Sintonización de Q con R fijo para el Filtro de Kalman}
\label{ap:kalman_tuning}
% ============================================================

En este apéndice se presenta el script utilizado para sintonizar la
covarianza del ruido de proceso del filtro de Kalman manteniendo fija la
covarianza de medición.

Se parametriza la matriz de proceso como \(Q = q I_n\) y se fija
\(R = \sigma_v^2\), donde \(\sigma_v\) se obtuvo a partir de mediciones
empíricas del sensor (planta en reposo). El escalar \(q\) se selecciona
mediante un criterio de consistencia estadística sobre la innovación
normalizada.

La innovación se define como \(\nu_k = y_k - \hat{y}_{k|k-1}\) y su varianza
teórica estacionaria como \(S = C P C^T + R\), donde \(P\) es la solución
estacionaria de Riccati. Se utiliza:

\[
\eta_k = \frac{\nu_k}{\sqrt{S}}
\]

y se escoge el valor \(q\) que aproxima \(\mathrm{var}(\eta_k)\approx 1\).

\subsection{Script de sintonización (RQ\_tuning\_fixedR.m)}
A continuación se incluye el código completo empleado para el barrido de \(q\)
en escala logarítmica, el cálculo de la ganancia estacionaria con \texttt{dlqe}
y el guardado de los parámetros resultantes en \texttt{RQ\_tuning\_fixedR.mat}.
...
\subsection{Parámetros exportados para el diseño LQG}

El script guarda el archivo \texttt{RQ\_tuning\_fixedR.mat} con los parámetros
utilizados posteriormente en el diseño del estimador y controlador:

\begin{itemize}
	\item \texttt{sigma\_v}: desviación estándar empírica del ruido de medición.
	\item \texttt{R}: covarianza del ruido de medición (\(R=\sigma_v^2\)).
	\item \texttt{best\_q}: escalar óptimo de la parametrización del ruido de proceso.
	\item \texttt{Q\_best}: matriz final de ruido de proceso (\(Q=qI_n\)).
	\item \texttt{L\_best}: ganancia estacionaria calculada con \texttt{dlqe}.
	\item \texttt{best\_P} y \texttt{best\_S}: covarianza estacionaria y varianza de innovación.
	\item \texttt{A,B,C,D,Ts\_target}: modelo discretizado usado en la sintonización.
\end{itemize}
