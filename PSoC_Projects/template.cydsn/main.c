...
#define T   (double)(1.0/300.0)
#define Ts  T
#define Kp  (double)0.5
#define Ti  (double)0.015
#define Td  (double)0.005
#define N   (double)8.0

/* Coeficientes precomputados (PID paralelo con filtro derivativo) */
#define alpha2 (Td*Ti/(T*T))
#define alpha1 (N*Ti/T)
#define beta2  (Kp*(N*Td*Ti + Td*Ti)/(T*T))
#define beta1  (Kp*(N*Ti + Td)/T)
#define beta0  (Kp*N)

#define U0  (alpha2 + alpha1)
#define U1  (-(2*alpha2 + alpha1))
#define U2  (alpha2)

#define E0  (beta2 + beta1 + beta0)
#define E1  (-(2*beta2 + beta1))
#define E2  (beta2)
...
static inline void actualizarEsfuerzo(void)
{
    ...
    /* Salida no saturada */
    float u_unsat = (float)((-U1*u_1 + -U2*u_2 + E0*e + E1*e_1 + E2*e_2)/U0);
